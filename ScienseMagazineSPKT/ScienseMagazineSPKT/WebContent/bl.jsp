				...
				<ul class="nav navbar-nav navbar-right">
					<li class="dropdown">
						...
						<ul class="dropdown-menu">
							...
							<c:if test="${cv == 'TacGia'}">
								<li><a href="tg_dspb.jsp">Bài viết đang trong quá trình
										phản biện</a></li>
...
SELECT * from rating where baiviet_id=? ;
            <sql:param value="${param.idbv}" />
				</sql:query>
				<c:set var="sumstart" value="${0}" scope="session" />
				<c:forEach var="row3" items="${result3.rows}">
					<c:set var="sumstart" value="${sumstart + row3.sosao}"
						scope="session" />
				</c:forEach>
				<c:set var="sumrating" value="${result3.rowCount}" scope="session" />
				
				

				<c:if test="${sumrating == 0}">
				<h2>Bài viết được đánh giá : <c:out value="${0}" />
						<span class="glyphicon glyphicon-star" style="color: #FFD700;"></span>/ ${sumrating} <span class="glyphicon glyphicon-user"></span></h2>
				</c:if>
				<c:if test="${sumrating != 0}">
				<h2>Bài viết được đánh giá : <fmt:formatNumber value='${sumstart/sumrating}'
						maxFractionDigits='1' />
						<span class="glyphicon glyphicon-star" style="color: #FFD700;"></span>/ ${sumrating} <span class="glyphicon glyphicon-user"></span></h2>
				</c:if>
            ...
            <sql:param value="${param.idbv}" />
				...
				<c:forEach var="row" items="${result.rows}">
					...
					<iframe frameborder="0"
						src="../ScienseMagazineSPKT/FileReadPdf?PDF=${sessionScope['idbv']}"
						style="height: 700px; width: 100%;"></iframe>
					<div class="panel panel-default">
						<br />
						<c:set var="empId" value="<%=asd%>" />
						<sql:query dataSource="${snapshot}" var="result2">
SELECT * from rating where baiviet_id=? and username = ? ;
            <sql:param value="${param.idbv}" />
							<sql:param value="${empId}" />
						</sql:query>
						<c:set var="check" value="${result2.rowCount}" />
						<c:if test="${not empty user}">
												<c:if test="${check == 0}">
							<div class="panel panel-default">
								<form action="z_rating.jsp" method="post">
        ...
        <label for="rating-input-1-1" class="rating-star"></label>
									...
									<div class="col-md-2">
										<input type="hidden" value="${sessionScope['idbv']}" name="idbv" />
...
SELECT * from binhluan where baiviet_id=? ;
            <sql:param value="${param.idbv}" />
						</sql:query>
						<c:forEach var="row" items="${result1.rows}">
							...
							<div class="col-md-12">
								<div class="container">
									<div class="panel panel-default nav navbar-nav nav-tabs">
										<div class="panel panel-heading">
											<h4 class="panel-tiile">
												<c:out value="${row.username}"></c:out>
											</h4>
											<c:set var="chucvu" scope="session" value="${row.cv}" />
											<c:if test="${chucvu == 'TongBienTap'}">Tổng biên tập -</c:if>
											<c:if test="${chucvu == 'PhanBienVien'}">Phản biện viên -</c:if>
											<c:if test="${chucvu == 'TacGia'}">Tác Giả -</c:if>
											<c:if test="${chucvu == 'DocGia'}">Đọc Giả -</c:if>
											<c:if test="${chucvu == 'Admin'}">Admin -</c:if>
											<c:if test="${chucvu == 'BienTapVien'}">Biên Tập Viên - </c:if>
											<c:out value="${row.ngay}"></c:out>
										</div>
										<div class="panel-body">
											<div class="container ">
												<div id="${row.binhluan_id}"></div>
												<input type="hidden" value='${row.binhluan}'
													id="${row.binhluan_id}s" />
												<script>
													var nd = document
															.getElementById('${row.binhluan_id}s').value;
													document
															.getElementById('${row.binhluan_id}').innerHTML = nd;
												</script>
											</div>
										</div>
									</div>
						...
						<c:if test="${not empty user}">
							<form action="z_bl.jsp" method="post">
								<div class="panel-body">
									<textarea name="editor1" id="editor1" rows="10" cols="80"></textarea>
								</div>
								<input type="hidden" value="${sessionScope['idbv']}" name="idbv" />
								<input type="hidden" value="<%=session.getAttribute("userid")%>"
									name="user" /> <input type="hidden" value="<%=cv%>" name="cv" />
