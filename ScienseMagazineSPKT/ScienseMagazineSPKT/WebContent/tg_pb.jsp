				...
				<ul class="nav navbar-nav navbar-right">
					<li class="dropdown">
						<button class="btn btn-default dropdown-toggle" type="button"
							...
							<span class="glyphicon glyphicon-cog"></span> Tác giả
						</button>
...
SELECT * from phanbien where baiviet_id=? ;
            <sql:param value="${param.idbv}" />
									...
									<div class="container">
										<div class="panel panel-default nav navbar-nav nav-tabs">
											...
											<div class="panel-body">
												<div class="container ">
													<div id="${row.phanbien_id}"></div>
													<input type="hidden" value='${row.phanbien}'
														id="${row.phanbien_id}s" />
													<script>
														var nd = document
																.getElementById('${row.phanbien_id}s').value;
														document
																.getElementById('${row.phanbien_id}').innerHTML = nd;
													</script>
												</div>
											</div>
										</div>
						...
						<input type="hidden" value="<%=session.getAttribute("userid")%>"
							name="user" /> <input type="hidden"
							value="<%=session.getAttribute("cv")%>" name="cv" /> <input
							class="btn btn-default" type="submit" value="Gửi" />
					</form>
					<form ng-app="myApp" ng-controller="validateCtrl" name="myForm"
						novalidate action="uploadServlet" method="post"
						enctype="multipart/form-data">
						<label>Update File</label> <input type="file" name="photo"
							class="form-control" id="UploadFile" onchange="myFunction()" /> <input
							type="hidden" value="${sessionScope['idbv']}" name="idbv" /> <input
							type="hidden" value="1" name="flag" /> <input id="myText"
							class="btn btn-default" type="submit" value="Update"
							title="Bạn phải chọn tệp" disabled="disabled"/>
					</form>
					<script>
					function myFunction() {
