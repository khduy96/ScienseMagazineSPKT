...
SELECT * from baiviet where tinhtrang = 'tbt' and dang='F' and trangthai = 'duyet' and dang='F';
</sql:query>
				<sql:query dataSource="${snapshot}" var="result2">
SELECT * FROM tapchikhspkt.vol ORDER BY vol_id DESC ;
</sql:query>
				<div class="container">
					<div class="panel panel-default navbar-nav">
						<div class="panel panel-heading">
							<div class="row">
								<div class="col-md-6">Danh sách bài đã duyệt</div>
								<div class="col-md-6" align="right">
									<form action="z_newvolume.jsp" method="post">
										<button class="btn btn-default" type="submit">
											<span class="glyphicon glyphicon-plus"></span> NEW VOLUME
										</button>
									</form>
								</div>
							</div>
						...
						<div class="panel-body">
							<div class="container ">
								<form action="z_dang.jsp" method="post">
										...
										<tbody>
											<c:forEach var="row" items="${result.rows}">
												<c:set var="idbv" value="${row.baiviet_id}"
													scope="session" />
												<input type="hidden" value="${row.baiviet_id}" name="id" />
												<tr>
													<td><c:out value="${row.tieude}" /></td>
													<td><select name="address"
														id="${sessionScope['idbv']}">
															<c:forEach var="row1" items="${result2.rows}">
																<c:set var="idvol" value="${row1.vol_id}"
																	scope="session" />
																<option value="${sessionScope['idvol']}">Volume
																	${sessionScope['idvol']}</option>
															</c:forEach>
													</select></td>
												</tr>
											</c:forEach>
									...
									<div align="right">
										<button class="btn btn-default" type="submit">Đăng
											bài</button>
									</div>
								</form>
							</div>
