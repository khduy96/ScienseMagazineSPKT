				...
				<ul class="nav navbar-nav navbar-right">
					<li class="dropdown">
						<button class="btn btn-default dropdown-toggle" type="button"
							...
							<span class="glyphicon glyphicon-cog"></span>Tổng biên tập <c:if test="${tong !=0 }"> <span class="badge">${tong}</span></c:if>
						</button>
						<ul class="dropdown-menu">
							<li><a href="tbt_dsyc.jsp">Danh sách yêu cầu đăng bài<c:if test="${db !=0 }"> <span class="badge">${bd}</span></c:if></a></li>
							<li><a href="tbt_dspb.jsp">Các bài viết đang phản biện</a></li>
							<li><a href="tbt_dsdpb.jsp">Các bài viết đã phản biện<c:if test="${pb !=0 }"> <span class="badge">${pb}</span></c:if></a></li>
							<li><a href="tbt_dsbt.jsp">Các bài viết đã biên tập<c:if test="${bt !=0 }"> <span class="badge">${bt}</span></c:if></a></li>
...
SELECT * from baiviet where (tinhtrang = 'btv') and dang='F' and (trangthai = 'dangbientap') ;
</sql:query>
				<div class="container">
					<div class="panel panel-default navbar-nav">
						<div class="panel panel-heading">Danh sách bài đã phản biện
						</div>
								...
								<table class="table table-striped">
									...
									<tbody>
										<c:forEach var="row" items="${result.rows}">
											...
											<tr>
												<td><a
													href="../ScienseMagazineSPKT/FileReadPdf?PDF=${sessionScope['idbv']}"><c:out
															value="${row.tieude}" /></a></td>
												<td>
													<button class="btn btn-default" type="submit"
														data-toggle="modal"
														data-target="#${sessionScope['idbv']}modal"
														style="float: left;">
														<span class="glyphicon glyphicon-option-horizontal"></span>
													</button>
												</td>
												<td align="right"><form action="tbt_pb.jsp" method="get">
													<input type="hidden" value="${sessionScope['iduser']}"
														name="idbv" /><input class="btn btn-default"
														type="submit" value="Xem phản biện" />
												</form></td>
												<td><form action="z_guipb.jsp"
														method="post">
														<input type="hidden" value="${sessionScope['idbv']}"
															name="id" /><input class="btn btn-default" type="submit"
															value="Phản biện lại" />
													</form></td>
												<td align="right"><form action="z_guibt.jsp" method="post">
														<input type="hidden" value="${sessionScope['idbv']}"
															name="id" /><input class="btn btn-default" type="submit"
															value="Gửi biên tập viên" />
													</form></td>
											</tr>
											<div class="modal fade bs-example-modal-lg"
												id="${sessionScope['idbv']}modal" role="dialog">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
														...
														<div class="modal-body">
															...
															<div id="${row.baiviet_id}"></div>
															<input type="hidden" value='${row.noidung}'
																id="${row.baiviet_id}s" />
															<script>
																				var nd = document
																						.getElementById('${row.baiviet_id}s').value;
																				document
																						.getElementById('${row.baiviet_id}').innerHTML = nd;
																			</script>
															<div class="row">
				...
				<div class="container">
					<div class="panel panel-default navbar-nav">
						<div class="panel panel-heading">Danh sách bài đang biên tập
						</div>
						<div class="panel-body">
							<div class="container ">
								<table class="table table-striped">
										...
										<c:forEach var="row" items="${result1.rows}">
											...
											<tr>
												...
												<td>
													<button class="btn btn-default" type="submit"
														data-toggle="modal"
														data-target="#${sessionScope['iduser']}modal"
														style="float: left;">
														<span class="glyphicon glyphicon-option-horizontal"></span>
													</button>
												</td>
											...
											<div class="modal fade bs-example-modal-lg"
												id="${sessionScope['iduser']}modal" role="dialog">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
														<div class="modal-header">
															<button type="button" class="close" data-dismiss="modal">&times;</button>
