...
SELECT * from members where position = 'TongBienTap' and trangthai='unblock';
</sql:query>
	<sql:query dataSource="${snapshot}" var="result1">
SELECT * from members where position='Admin'and trangthai='unblock';
</sql:query>
	<sql:query dataSource="${snapshot}" var="result2">
SELECT * from members where position='PhanBienVien'and trangthai='unblock'and trangthai='unblock';
</sql:query>
	<sql:query dataSource="${snapshot}" var="result3">
SELECT * from members where position='BienTapVien'and trangthai='unblock';
</sql:query>
	<sql:query dataSource="${snapshot}" var="result4">
SELECT * from members where position='TacGia'and trangthai='unblock';
</sql:query>
	<sql:query dataSource="${snapshot}" var="result5">
SELECT * from members where position='DocGia'and trangthai='unblock';
</sql:query>
	<sql:query dataSource="${snapshot}" var="result6">
SELECT * from members where trangthai='block';
</sql:query>
<sql:query dataSource="${snapshot}" var="result7">
SELECT * from members where position='DocGia'and trangthai='unblock' and yeucau='T';
</sql:query>
	<sql:query dataSource="${snapshot}" var="resultbv">
SELECT * from baiviet;
</sql:query>
	...
	<div class="container-fluid">
		...
		<div class="tab-content">
			<div id="dstk" class="tab-pane fade in active">
				<div class="container-fluid">
					<ul class="nav navbar-nav nav-tabs">
						<li class="active"><a data-toggle="tab" href="#menu0">Tổng
								Biên Tập</a></li>
						<li><a data-toggle="tab" href="#menu1">Admin</a></li>
						<li><a data-toggle="tab" href="#menu2">Phản Biện Viên</a></li>
						<li><a data-toggle="tab" href="#menu3">Biên Tập Viên</a></li>
						<li><a data-toggle="tab" href="#menu4">Tác Giả</a></li>
						<li><a data-toggle="tab" href="#menu5">Đọc Giả</a></li>
						<li><a data-toggle="tab" href="#menu7">Yêu cầu</a></li>
						<li><a data-toggle="tab" href="#menu6">Block</a></li>
					</ul>
					<div class="tab-content">
						<div id="menu0" class="tab-pane fade in active">
							<table class="table table-striped">
								...
								<c:forEach var="row" items="${result.rows}">
									<c:set var="iduser" value="${row.id}" scope="session" />
									<tr>
										<td><c:out value="${row.id}" /></td>
										<td><c:out value="${row.username}" /></td>
										<td><c:out value="${row.email}" /></td>
										<td><select id="${sessionScope['iduser']}">
												<option value="TongBienTap">Tổng Biên Tập</option>
												<option value="Admin">Admin</option>
												<option value="BienTapVien">Biên Tập Viên</option>
												<option value="PhanBien">Phản Biện</option>
												<option value="TacGia">Tác Giả</option>
								...
								<c:forEach var="row" items="${result3.rows}">
									...
									<tr>
										...
										<td><select id="${sessionScope['iduser']}">
												...
												<option value="Admin">Admin</option>
												<option value="TongBienTap">Tổng Biên Tập</option>
												<option value="PhanBien">Phản Biện Viên</option>
												<option value="TacGia">Tác Giả</option>
												<option value="DocGia">Đọc Giả</option>
												<option value="block">Block</option>
										</select></td>
